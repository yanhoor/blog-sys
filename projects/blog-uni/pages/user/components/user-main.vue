<template>
  <view>
    <SkeletonUserMain v-if="loading && !userInfo" />

    <view v-else class="me-main">
      <view class="top-wrapper">
        <image
          v-if="userInfo.profileCardBg"
          class="bg-img"
          :src="imageHost + userInfo.profileCardBg"
          mode="aspectFill"
          @click="handleClickImage(userInfo.profileCardBg)"
        />
        <view v-else class="bg-img default" />
        <view class="info-card">
          <view class="user-info-container">
            <view class="user-left">
              <UserAvatar
                ...
                :clickable="false"
              />
              <UserName font-size="22" :user="userInfo" />
              <uni-icons
          ...
          <view class="user-stats-container">
            <view
              class="stats-item"
              @click="
                handleToPage('/pages/followings/followings?userId=' + userId)
              "
            >
              <view class="stats-num">
                {{ userInfo.followingCount }}
              </view>
              <view class="stats-desc"> 关注 </view>
            </view>
            <view
              class="stats-item"
              @click="
                handleToPage('/pages/followers/followers?userId=' + userId)
              "
            >
              <view class="stats-num">
                {{ userInfo.followerCount }}
              </view>
              <view class="stats-desc"> 粉丝 </view>
            </view>
            <view class="stats-item">
              <view class="stats-num">
                {{ userInfo.likedCount }}
              </view>
              <view class="stats-desc"> 获赞 </view>
            </view>
          </view>
          <view v-if="userInfo.introduce" class="introduce">
            {{ userInfo.introduce }}
          </view>
        </view>
      </view>
      ...
      <view class="content-section">
        <view v-if="imageTotal" class="section-container">
          ...
          <view class="action-section">
            <view
              ...
              @click="
                handleToPage(
                  '/pages/user-all-image/user-all-image?id=' + userId
                )
              "
            >
              全部
        ...
        <UserImageWall
          :user-id="userInfo.id"
          @fetch-end="imageTotal = $event.total"
        />
      </view>
      ...
      <view class="content-section">
        <view class="section-container">
          <view class="title-section">
            <view class="title"> 全部博客 </view>
            <view class="title-num">
              {{ postTotal }}
            </view>
          </view>
          <view class="action-section">
            <uni-icons type="search" size="20" @click="showSearch = true" />
          </view>
        </view>
        <PostList
          class="post-list"
          :page-url="'/user/' + userId"
          :search-params="{ uid: userInfo.id, keyword }"
          @fetch-end="postTotal = $event.total"
        />
      </view>

      <view v-if="showSearch" class="search-wrapper">
        <uni-search-bar
          v-model="keyword"
          focus
...
export default {
  ...
  methods: {
    ...
    async getUserInfo() {
      const { success, result, msg } = await Http.post('/user/' + this.userId)
      if (success) {
