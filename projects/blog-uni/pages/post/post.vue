<template>
  <view class="post-page">
    <SkeletonPostDetail v-if="loading && !postInfo" />

    <view v-else>
      <uni-card margin="5px" spacing="0" class="card">
        <view class="post-content-wrapper">
          <view class="post-top">
            <UserAvatar :user="postInfo.createBy" :size="42" />
            <view class="item-user">
              <UserName font-size="18" :user="postInfo.createBy" />
              <YTime
                class="item-time"
                type="format"
          ...
          />
          <MediaList :list="postInfo.medias" />
          <PostAddress :address="postInfo" />
    ...
    <uni-card margin="5px" class="tab-card">
      <view v-if="postInfo" class="tab-container">
        <view
          class="tab-item"
      ...
      <view class="tab-content">
        <UserList
          v-if="currentTab === 1"
          page-url="pages/post/post/1"
          :search-params="{ blogId: Number(postId) }"
          url="/blog/actionUserList/1"
        />
        <CommentList
          v-if="currentTab === 2"
          class="post-comment"
          :blog-id="Number(postId)"
        />
        <UserList
          v-if="currentTab === 3"
          page-url="pages/post/post/3"
          :search-params="{ blogId: Number(postId) }"
    ...
    <CommentReplyForm v-model:show="showReply" :blog-id="Number(postId)" />
...
export default {
  ...
  methods: {
    async initPage() {
      this.loading = true
      try {
        await Promise.all([this.getPostInfo()])
    ...
    async getPostInfo() {
      const { success, result, msg } = await Http.post(urls.blog_info, {
        id: this.postId
      })
      if (success) {
