<template>
  <view v-if="!myInfo" class="me-page-empty" />

  <view v-else class="me-page">
    <uni-card spacing="0" margin="5px">
      <view class="top-card-container">
        <view
          class="user-wrapper"
          @click="handleToPage('/pages/user/user?id=' + myInfo.id)"
        >
          <UserAvatar disabled :user="myInfo" :size="60" />
          <view class="user-info-container">
            <view class="user-name">
              {{ myInfo.name }}
            </view>
            <view class="stats-container">
              <view class="stats-item"> {{ myInfo.followingCount }} 关注 </view>
              <view class="stats-item"> {{ myInfo.followerCount }} 粉丝 </view>
            </view>
          </view>
          <uni-icons type="forward" size="32" color="#bbb" />
        </view>

        <view class="action-container">
          <view
            class="action-item"
            @click="handleToPage('/pages/profile/profile')"
          >
            <uni-icons custom-prefix="iconfont" type="y-edit" />
            <view>编辑资料</view>
          </view>
    ...
    <uni-card spacing="0" margin="5px">
      <uni-list :border="false">
        ...
        <uni-list-item
          ...
          show-extra-icon
          show-arrow
          :extra-icon="{ type: 'hand-up-filled' }"
          to="/pages/my-like/my-like"
          title="我的点赞"
        />
        <uni-list-item
          clickable
          show-extra-icon
          show-arrow
          :extra-icon="{ type: 'chatbubble-filled' }"
          to="/pages/my-comment/my-comment"
          title="我的评论"
        />
        <uni-list-item
          clickable
          ...
          to="/pages/user-group-manage/user-group-manage"
          title="我的分组"
        />
      </uni-list>
    </uni-card>
