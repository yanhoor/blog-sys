<template>
  <view v-if="myInfo">
    <uni-segmented-control
      ...
      style-type="text"
      active-color="#18a058"
      @click-item="handleChangeTab"
    />
    <view class="content">
      <NotificationComment v-if="currentTab === 0" />
      <NotificationLike v-if="currentTab === 1" />
      <NotificationCollect v-if="currentTab === 2" />
      <NotificationSystemAudit v-if="currentTab === 3" />
    </view>
  </view>
</template>
...
export default {
  ...
  computed: {
    ...
    tabList() {
      ...
      const tab4 = store.unreadAudit
        ? `系统审核(${store.unreadAudit})`
        : '系统审核'
