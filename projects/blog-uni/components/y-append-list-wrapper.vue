...
<template>
  <view>
    ...
    <slot v-if="loadStatus === 'loading' && !pageList.length" name="skeleton" />
    <ResultError v-else-if="fetchError" @refresh="handleRefresh" />
    <ResultEmpty v-else-if="!pageList.length" @refresh="handleRefresh" />
    <uni-load-more
      v-else
      :status="loadStatus"
      :content-text="{ contentdown: '上拉或点击加载更多' }"
...
export default {
  ...
  methods: {
    async getList(reset) {
      ...
      try {
        const { result, success, msg } = await Http.post(
          this.url,
          this.fetchParams
        )
