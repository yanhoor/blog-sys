<template>
  <view v-if="myInfo" class="comment-reply-form">
    <uni-popup
      ref="replyPopupRef"
      type="bottom"
    ...
    >
      <view class="reply-container">
        <UserAvatar :user="myInfo" :clickable="false" :size="36" />
        <uni-easyinput
...
export default {
  ...
  computed: {
    ...
    placeholder() {
      return this.currentReplyItem
        ? `回复@${this.currentReplyItem.createBy.name}:`
        : '写评论...'
    }
  },
  methods: {
    ...
    handlePostForm() {
      if (this.currentReplyItem) {
        ...
        this.replyForm.topCommentId =
          this.currentReplyItem.topCommentId || this.currentReplyItem.id
      } else {
