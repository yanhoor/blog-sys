  ...
  <div v-else class="home-container flex h-full w-full flex-col gap-12">
    <div class="flex w-full gap-6">
      <NumberBoard
        class="flex-1"
        title="博客总数"
        :count="totalCount.blogCount"
      />
      <NumberBoard
        class="flex-1"
        title="用户总数"
        :count="totalCount.userRegisterCount"
      />
      <NumberBoard
        class="flex-1"
        title="今日新增用户数"
        :count="totalCount.userRegisterTodayCount"
      />
      <NumberBoard
        class="flex-1"
        title="今日活跃用户数"
...
async function InitPage() {
  try {
    loading.value = true
    await Promise.all([getWeekCountDetail(), getTotalCount()])
    loading.value = false
  } catch (e) {
    loading.value = false
  }
