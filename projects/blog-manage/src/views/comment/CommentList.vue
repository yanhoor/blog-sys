<template>
  <ListWrapper>
    ...
    <template #table>
      <el-table border stripe :data="pageList" height="100%" v-loading="listLoading">
        ...
        <el-table-column key="status" label="状态">
          <template #default="{ row }">
            <div class="flex gap-[6px]">
              ...
              <el-tag v-else-if="row.auditStatus === 2" type="warning">审核不通过</el-tag>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column key="auditedAt" label="审核时间">
          <template #default="{ row }">
            <div v-if="row.auditedAt">{{dayjs(row.auditedAt).format('YYYY-MM-DD HH:mm:ss')}}</div>
            <div v-else>-</div>
          </template>
        </el-table-column>
