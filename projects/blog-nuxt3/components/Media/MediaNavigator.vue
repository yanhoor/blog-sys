<template>
  <div class="w-full relative">
    <div class="w-full overflow-hidden">
      <div class="flex transition-all -ml-[6px]" :style="{'transform': `translateX(-${(currentPage - 1) * 100}%)`}">
        <div class="pl-[6px] relative shrink-0" :style="`width: calc(100% / ${pageSize})`" v-for="(file, index) of list" :key="file.url">
          <div class="image-item-container">
            <MediaImgView alt="图像" class="image-item border-solid border-green-500" :url="file.url" ratio="80"/>
            <div class="list-item-mask border-2 border-green-500" v-if="modelValue === file"></div>
            <div class="list-item-mask bg-gray-600 opacity-30 cursor-pointer" v-if="modelValue !== file" @click="handlePreview(file, index)"></div>
          </div>
        </div>
      </div>
    </div>
    <div class="flex items-center cursor-pointer absolute -left-[20px] h-full top-0" @click="handleNextImagePage(-1)" v-if="currentPage > 1">
      <n-icon :component="ChevronLeft24Regular" size="24"/>
    </div>
    <div class="flex items-center cursor-pointer h-full top-0 absolute -right-[20px]" @click="handleNextImagePage(1)" v-if="currentPage < imageTotalPage">
