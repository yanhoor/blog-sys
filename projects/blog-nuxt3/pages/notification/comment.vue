    ...
    <template #default="{ notification }">
      <div class="whitespace-nowrap max-w-full flex items-center gap-[6px]">
        ...
        <template v-if="notification.comment.replyComment">
          <span
            class="text-green-700 font-semibold cursor-pointer text-[18px]"
            @click="navigateTo({ path: '/user/' + notification.createById })"
            >{{ notification.createBy.name }}</span
          >
          回复了您的评论
          <span
            class="text-gray-500 px-[6px] border custom-border rounded truncate max-w-full bg-gray-200 dark:bg-gray-600 dark:text-gray-300"
            >{{ notification.comment.replyComment.content }}</span
          >：
        </template>
        <template v-else>
          <span
      ...
      <ExpandableContent
        class="pt-[12px] font-semibold w-full"
